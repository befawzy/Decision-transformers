...
class DecisionTransformer(DecisionTransformerModel):
    ...
    def __init__(self, config):
        ...
        self.predict_action = nn.Sequential(
            nn.Linear(config.hidden_size, config.act_dim), nn.Softmax(dim=2))
    ...
    def forward(self, **kwargs):
        ...
        action_targets = action_targets.reshape(-1,
                                                act_dim)[attention_mask.reshape(-1) > 0]
        # adding loss function for the sake of training
        loss_fn = nn.CrossEntropyLoss()
        loss = loss_fn(action_preds, action_targets)
        return {"loss": loss}

    def original_forward(self, *kwargs):
        state_preds, action_preds, return_preds = super().forward(*kwargs, return_dict=False)
        action_preds=torch.argmax(action_preds.reshape(-1,3),dim=1).reshape(1,-1)
